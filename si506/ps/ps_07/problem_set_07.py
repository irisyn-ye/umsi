...
def filter_movie_by_genre(movies, genre):
    """
    Loop through movies (list) and check if genre (str) is included in the dictionary key: 'Genre'.
...
def get_jumpscare_data(jumpscare_data, movie_name):
    ...
    Parameters:
        jumpscare_data (list): information about the predicted outcome of a basketball game
    ...
    Returns:
        tuple: A tuple with the first value representing a jump count and the second value
            representing a jump scare rating.
    """
    for jumpscare_data_obj in jumpscare_data:
        if movie_name.lower() == jumpscare_data_obj['Movie Name'].lower(): 
            jumpscare_count = jumpscare_data_obj['Jump Count']
            jumpscare_rating = jumpscare_data_obj['Jump Scare Rating']
            return (jumpscare_count, jumpscare_rating)

# PROBLEM 5
def filter_movies(movies, key_name, lower_limit, upper_limit):
    """
    Loops through a list of dictionaries and checks if the defined to key is between
    the integers passed to the lower_limit and upper_limit parameters.
    ...
    filtered_movie = []
    for movie in movies: 
        if (movie[key_name] >= lower_limit and movie[key_name] <= upper_limit): 
...
def search_movie_plot(movie, search_terms):
    ...
    search term appears in the movie's plot. If the search term is in the movie plot this function returns True,
    if the search term is not in the movie's plot, this function will return False
    ...
    for search_term in search_terms: 
        if search_term.lower() in movie['Plot'].lower(): 
            return True
        else:
            return False
...
def extract_kv_pair(movie, key_name):
    ...
    Uses a conditional statement to check if the key is the same as the passed in key_name.
...
def total_runtime_hrs(data):
    """
    Using the `dict.values()` method, this function takes a dictionary of runtimes and returns the total runtime converted from mins to hours.
    The return value is a whole a number (no fractional component).
...
def main():
    ...
    print("\nProblem 04")
    for horror_movie in horror_movies: 
        jumpscare_count, jumpscare_rating = get_jumpscare_data(jumpscare_data, horror_movie['Title'])
        horror_movies.append({'Jumpscare_count': jumpscare_count, 'Jumpscare_rating': jumpscare_rating})
    # jumpscare_count, jumpscare_rating = get_jumpscare_data(jumpscare_data, horror_movies['Title'])

    #print(f'Horror movie with jump scare data: {horror_movies[0]}')

    # PROBLEM 5
    print("\nProblem 05")
    short_movies = filter_movies(horror_movies, 'Runtime', 30, 90)
    #print(f'Shorter horror movies: {short_movies[0]}')
    ...
    for horror_movie in horror_movies: 
        if search_movie_plot(horror_movie, ghost_terms) == True: 
            ghost_movies.append(horror_movie)

    #print(f'Ghost movies: {ghost_movies}')

    high_jumpscare_ghost_movies = filter_movies(ghost_movies, 'Runtime', 10, 30)

    #  PROBLEM 7
    print("\nProblem 07")
    movie_runtimes = {}
    for high_jumpscare_ghost_movie in high_jumpscare_ghost_movies: 
        movie_title, movie_runtime = extract_kv_pair(high_jumpscare_ghost_movie, 'Runtime')
        movie_runtimes.append({movie_title: movie_runtime})
    ...
    #print(f'Total runtime for ghost movies with high number of jump scares: {total_runtime}')
