...
def request_resource_details(base_url, category=None, params=None):
    ...
    categories = ['planets', 'people', 'starships', 'species', 'vehicles']
...
def main():
    ...
    for key, people_info in people_details.items():
        ...
        if species_name not in species_order.keys():
            species_order[species_name] = [key]
        else:
            species_order[species_name].append(key)
    
    # print(type(people_info['species'][0]['name']))
    for species_order_key in species_order.keys():
        species_count_obj = (species_order_key, len(species_order[species_order_key]))
        species_count.append(species_count_obj)
    # print(species_count)
    ...
    planet_details['Dagobah']['starships'] = x_wing
