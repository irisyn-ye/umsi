...
# 1.0 WARMUP
ev_station_count = len(station_data[1:]) # TODO Return length

# print(f"\n1.0 EV station count = {ev_station_count}")

# 1.1 CHALLENGE 01

headers = station_data[0] # TODO access headers
...
for i in range(len(station_data[1:])):
    ...
    if station_data[i][idx] == 'A2DDA': 
        station_data[i][idx].replace("A2DDA", "Ann Arbor Downtown Development Authority -")
...
idx = None # TODO return headers ev_connector_types index to use in loop below
i = 1
