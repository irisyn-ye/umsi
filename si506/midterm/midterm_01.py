...
def count_vaccinated(counties, headers, header_items):
    ...
    for county in counties: 
        accumulator_a += get_attribute(county, headers, header_items[0])
        accumulator_b += get_attribute(county, headers, header_items[1])
...
def get_ur_scheme(ur_codes, county_name):
    ...
    for ur_code in ur_codes: 
        if ur_code[-1].lower() == county_name.lower():
            return tuple(ur_code[2], convert_to_int(ur_code[-2]), ur_code[-1])  # TODO Implment
...
def main():
    ...
    for i in range(len(case_counties)): 
        if case_counties[i][1].lower() in region:
            region_cases.append(case_counties[i][1].lower())
    ...
    washtenaw_vax_series_complete_pct = calculate_vax_pct(washtenaw_vax_series_complete, washtenaw_pop_total, precision=2) # TODO call function
    ...
    genesee_vax_series_complete_5to17_pct = calculate_vax_pct(genesee_vax_series_complete_5to17, genesee_pop_total_5to17, precision=None) # TODO call function
    ...
    vax_total_18plus_pct = calculate_vax_pct(vax_total_18plus, census_total_18plus, precision=2) # TODO call function
    ...
    for i in range(len(vax_counties)):
        ...
        cbsa_title, ur_code, ur_code_nameget_ur_scheme = get_ur_scheme(ur_schemes, county_name)
        vax_counties[i] += cbsa_title
        vax_counties[i] += ur_code
        vax_counties[i] += ur_code_nameget_ur_scheme

    # TODO insert ur_headers elements into vax_headers
    vax_headers.append(ur_headers)
    # print(vax_counties)
    ...
    write_csv("stu-mi_ur_county_vax_levels.csv", vax_counties, vax_headers)
